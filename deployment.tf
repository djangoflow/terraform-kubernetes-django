resource "kubernetes_deployment_v1" "deployment" {
  depends_on       = [kubernetes_namespace_v1.namespace, kubernetes_secret_v1.secrets]
  ...
  spec {
    ...
    strategy {
      type = var.rolling_update != null ? "RollingUpdate" : "Recreate"

      dynamic "rolling_update" {
        for_each = var.rolling_update != null ? flatten([var.rolling_update]) : []
        content {
